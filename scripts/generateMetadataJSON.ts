...
const main = () => {
  ...
  const secondImage = readFileSync("assets/Marsh.png").toString("hex");
  const thirdImage = readFileSync("assets/Sprinkle.png").toString("hex");
    ...
    [
      {
        width: 1024,
        height: 1024,
        url: "ipfs://Qmf4r4LcBG6XsRzQDTjEjNcxcLhUt4xyTt3aZiFgbBwUZo/Marsh.png",
        verification: {
          method: "keccak256(bytes)",
    ...
    [
      {
        ...
        verification: {
          method: "keccak256(bytes)",
          hash: ethers.keccak256(`0x${thirdImage}`),
        },
      },
    ],
  ];
